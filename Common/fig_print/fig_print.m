function fig_print(fig, figname, dim, crop, magn_factor, bg_color)
% 
...
% - DIM        : 1x2 numeric array with the image dimensions. If DIM=[],
%   the current figure size times MAGN_FACTOR will be used.
% - CROP       : 1 (crop image) or 0 (do not crop). Homogeneous edges will
%   be removed. [Default = 0]
% - MAGN_FACTOR: magnification factor (any positive number). Only effective
%   when DIM=[]. [Default = 1]
% - BG_COLOR   : If specified, the figure window background color will be
%   changed. You can specify a color using a three-element vector of RGB
%   values or one of the MATLAB predefined names (short name: 'y', 'm',
%   'c', 'r', 'g', 'b', 'w', 'k')
% 
% 
%OUTPUT
...
% Copyright (C) 2013 Guilherme Coco Beltramini
...
%Changelog
%--------------------------------------------------------------------------
% 2014-Jul-15, 02:26 pm: added the option to change the figure background
%   color
...
oldpapertype = get(fig,'PaperType');
% Standard paper size: usletter, uslegal, A0, A1, A2, A3, A4, A5, B0, B1,
% B2, B3, B4, B5, arch-A, arch-B, arch-C, arch-D, arch-E, A, B, C, D, E,
% tabloid
% When other paper properties are changed, 'PaperType' is also changed.
...
if ~isempty(bg_color)
    set(fig, 'Color', bg_color) % background color
end
