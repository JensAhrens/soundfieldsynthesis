function [ Xdb, Xmax_dB, t, f ] = myspectrogram3d(x,nfft,fs,window,noverlap,doplot,dbdown,yscale)
...
if nargin<8, yscale='lin'; end
if nargin<7, dbdown=60; end
...
X_lin = zeros(nfft,nframes); % allocate output spectrogram
...
%f = 0.001*(0:nfft/2-1)*fs/nfft;
f = (0:nfft/2-1)*fs/nfft;
Xdb = 20*log10(abs(X_lin(1:end/2,:)));
Xmax_dB = max(max(Xdb));
% Clip lower limit to -dbdown dB so nulls don't dominate:
clipvals = [Xmax_dB-dbdown,Xmax_dB];
[ T, F ] = meshgrid( t, f );
